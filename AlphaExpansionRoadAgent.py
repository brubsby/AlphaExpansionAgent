...
env = gym.make('AlphaExpansionRoad-v0').unwrapped
...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, input_depth, actions):
        ...
        no_action_hidden_layer_size = 16
        self.no_action_hidden = nn.Linear(linear_input_size, no_action_hidden_layer_size)
        self.no_action_bn = nn.BatchNorm1d(no_action_hidden_layer_size)
        self.no_action_head = nn.Linear(no_action_hidden_layer_size, 1)
    ...
    def forward(self, x):
        ...
        return torch.cat((x.view(x.size(0), -1),
                          self.no_action_head(F.selu(self.no_action_hidden(x.view(x.size(0), -1))))), 1)
...
EPS_DECAY = 50
TARGET_UPDATE = 10
HEIGHT = env.game.map.CHUNK_HEIGHT
WIDTH = env.game.map.CHUNK_WIDTH
DEPTH = 8
ACTIONS = env.action_space.n
env.ravel = False
writer = SummaryWriter()

policy_net = DQN(HEIGHT, WIDTH, DEPTH, ACTIONS).to(device)
target_net = DQN(HEIGHT, WIDTH, DEPTH, ACTIONS).to(device)
...
for i_episode in range(num_episodes):
    ...
    state = torch.from_numpy(env.reset()).permute(2, 0, 1).unsqueeze(0).type(torch.cuda.FloatTensor)
    score = 0.0
    for t in count():
        # Select and perform an action
        action = select_action(state)
        next_state, reward, done, _ = env.step(action)
        next_state = torch.from_numpy(next_state).permute(2, 0, 1).unsqueeze(0).type(torch.cuda.FloatTensor)
